        ...
        await db.Database.ExecuteSqlRawAsync(@"
            ...
            BEGIN
                ...
                VALUES (1, 'openai/gpt-4o-mini',
                    ...
                    عن الطلبات والتتبع والإلغاء والاسترجاع باستخدام الأدوات المتاحة لك، مش بالتخمين:
                    - لو العميل بيسأل عن أوردر من غير ما يديك رقمه، استخدم أداة get_customer_orders الأول
                      عشان تعرف تحدد هو قاصد أوردر رقم كام.
                    - لو معاك رقم الأوردر وعايز تعرف حالته أو تتبعه، استخدم أداة track_order.
                    - لو العميل طلب يلغي أوردر، استخدم أداة cancel_order (بتشتغل بس لو الأوردر لسه
                      Pending أو Accepted؛ لو رفضت لمرحلة متأخرة اقترح عليه شكوى أو تحويل لأدمن).
                    - لو حسيت إن العميل بيشتكي من مشكلة حقيقية أو عايز يسترجع منتج بعد التوصيل
                      (طلب متأخر جدًا، منتج فاسد أو ناقص، سلوك سيء من مندوب، مبلغ اتخصم غلط... إلخ)
                      استخدم أداة create_complaint عشان تسجل شكوى/طلب استرجاع رسمي باسمه.
                    - ولو الموضوع معقد أو محتاج قرار بشري أو العميل نفسه طلب يتكلم مع حد حقيقي،
                      استخدم أداة escalate_to_admin.', 512);
            END
        ");
